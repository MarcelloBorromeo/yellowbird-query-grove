      ...
      <div className="space-y-4">
        ...
        <div className={cn("glass-card rounded-lg p-4 transition-all duration-500", isProcessing ? "border-accent/30" : hasError ? "border-destructive/30" : "border-green-500/30")}>
          <div className="flex items-center justify-between mb-2">
            ...
            {!isProcessing && !hasError && sqlQuery && <div className="flex items-center space-x-2">
          ...
          {/* SQL Output */}
          {sqlQuery || modifiedSql ? <div className="mt-2 pl-8">
              <div className="relative">
                {isEditMode ? <div className="relative">
                    <textarea value={modifiedSql} onChange={e => setModifiedSql(e.target.value)} className="w-full text-xs font-mono bg-secondary/30 p-3 rounded-md overflow-x-auto max-h-72 min-h-[120px] scrollbar-thin focus:ring-1 focus:ring-yellowbird-500 focus:outline-none" />
                    <div className="absolute top-2 right-2 flex space-x-2">
                      <button className="text-xs bg-yellowbird-500 hover:bg-yellowbird-600 text-white px-3 py-1 rounded flex items-center space-x-1" onClick={handleRunModifiedSql}>
                        <PlayCircle className="h-3 w-3" />
                        <span>Run</span>
                      </button>
                      <button className="text-xs bg-secondary/50 hover:bg-secondary px-2 py-1 rounded text-muted-foreground hover:text-foreground transition-colors" onClick={() => navigator.clipboard.writeText(modifiedSql || '')}>
                        Copy
                      </button>
                    </div>
                    ...
                    <div className="absolute top-2 right-2">
                      <button className="text-xs bg-secondary/50 hover:bg-secondary px-2 py-1 rounded text-muted-foreground hover:text-foreground transition-colors" onClick={() => navigator.clipboard.writeText(sqlQuery || '')}>
                        Copy
                      </button>
                    </div>
                  </div>}
        ...
        {/* Database Query */}
        {sqlQuery && !isProcessing && !hasError && <div className="glass-card rounded-lg p-4 animate-fade-in">
            <div className="flex items-center space-x-2 mb-2">
              <div className="bg-secondary/50 p-1.5 rounded-md">
